...
class ProgressWidget extends StatefulWidget {
  ProgressWidget(
      ...
      this.title, this.durationTime, this.radius, this.fontSize, this.width})
      : super(key: key);
...
class _ProgressWidgetState extends State<ProgressWidget>
  ...
  Widget build(BuildContext context) {
    final dimension = (SizeConfig.screenWidth) * widget.width;
    if (activeColor == null) {
        ...
        children: <Widget>[
          Stack(
            ...
            children: <Widget>[
              ...
              Text(
                "${widget.value} %",
                style: TextStyle(fontSize: widget.fontSize, fontFamily: 'AA-GALAXY',color: black),
              ),
            ],
          ),
          SizedBox(height: 50),
          Text(
            widget.title,
            style: TextStyle(fontSize: widget.fontSize, color: button,fontFamily: 'AA-GALAXY'),
          )
        ],
      ),
    );
...
class ProgressPainter extends CustomPainter {
  ...
  ProgressPainter(this.value, this.activeColor, this.backgroundColor, this.radius);
  @override
  void paint(Canvas canvas, Size size) async {
    ...
    Paint inActive = new Paint()
      ...
      ..strokeWidth = 8;
    canvas.drawArc(
        Rect.fromCircle(center: center1, radius: radius), 0, 180, false, inActive);

    canvas.drawArc(Rect.fromCircle(center: center1, radius: radius), -90 * pi / 180,
